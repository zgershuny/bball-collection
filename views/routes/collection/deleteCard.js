...
router.post('/deleteCard', function(req, res, next){
    ...
    // find the card
    mysql.connection.query("SELECT cardID, saleID, seriesID, teamID, playerID, featureID, parallelID, slabID, '#d', 'out of', note FROM fact_card WHERE cardID=?", [context.cardID],
        function(err, result){
            if(err){
                next(err);
                return;
            ...
            // delete the card
            mysql.connection.query("DELETE FROM `fact_card`, `dim_sale`, `dim_slab`\n" +
                "USING `fact_card`\n" +
